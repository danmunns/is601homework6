...
def generate_test_data(num_records):
    ...
    for _ in range(num_records):
        num_1 = Decimal(fake.random_number(digits=2))
        num_2 = Decimal(fake.random_number(digits=2)) if _ % 4 != 3 else Decimal(fake.random_number(digits=1))
        ...
        if operation_func == divide:
            num_2 = Decimal('1') if num_2 == Decimal('0') else num_2
        
        try:
            if operation_func == divide and num_2 == Decimal('0'):
                expected = "ZeroDivisionError"
            else:
                expected = operation_func(num_1, num_2)
        except ZeroDivisionError:
            expected = "ZeroDivisionError"
        
        yield num_1, num_2, operation_name, operation_func, expected
...
def pytest_generate_tests(metafunc):
    ...
    if {"num_1", "num_2", "expected"}.intersection(set(metafunc.fixturenames)):
        ...
        modified_parameters = [(num_1, num_2, op_name if 'operation_name' in metafunc.fixturenames else op_func, expected) for num_1, num_2, op_name, op_func, expected in parameters]
